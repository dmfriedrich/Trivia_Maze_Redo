...
public class Maze implements Serializable {  
  ...
  private Tom omniTom = new Tom();
  
  private boolean foundTom = false;
  ...
  public Maze() {
	  myPlayer = new Player();
	  mySQL = new SQL();
	  mySQL.randomizeQuestions();
	  my2DMaze = new Room[myRows + 2][myColumns + 2]; // makes 2D array w/ rows and columns
	    // +2 for a buffer
	  
	  makeMazeOfRooms(false);
	  
	  
  }
  
  public Maze(boolean myPasscode) {
	  myPlayer = new Player();
	  mySQL = new SQL();
	  mySQL.randomizeQuestions();
	  my2DMaze = new Room[myRows + 2][myColumns + 2]; // makes 2D array w/ rows and columns
...
public int getRows() {
  ...
  public void makeMazeOfRooms(boolean presentation) {
	if (presentation) {
		for (int i = 1; i <= myRows+1; i++)
	    {
	       for (int j = 1; j <= myColumns+1; j++)
	       {
	          ...
	          for (int m = 0; m < 4; m++) {
	        	  my2DMaze[i][j].cardinalDoors[m] = new Door(false);
	        	  }
	          if (i == 2 && j == 2) {
        		  my2DMaze[i][j].setTom(true);
	          }
	          my2DMaze[i][j].wallCheck();
	       }
	    }
	}
	else {
    for (int i = 1; i <= myRows+1; i++)
    {
       for (int j = 1; j <= myColumns+1; j++)
       {
          my2DMaze[i][j] = new Room(j, i, i/myRows, j/myColumns);
          for (int m = 0; m < 4; m++) {
        	  my2DMaze[i][j].cardinalDoors[m] = my2DMaze[i][j].createDoor(mySQL.getRandQuestion());
          }
          my2DMaze[i][j].wallCheck();
       }
    }
  ...
  public String startMaze() {
	  ...
	  while (!(my2DMaze[myRows][myColumns].ifPlayer.equals("[o]")) && stillPossible) {
	      ...
	      if (dr.getDoorLock() == true) {
	       ...
	       myPlayer.moveSuccess(move);
	       if (my2DMaze[myPlayer.getY()][myPlayer.getX()].isTom() && foundTom == false) {
	    	   omniTom.tomIntroPrompt();
	       }
	      }
